...
- ✅ **P2 BUG FIXED: Backend 500 on filtered requests** — 2026-09-15.
  ...
  - **Deploy note:** backend-only change — redeployed via `main` (`5a29324` on develop → merge `34f26be` → pushed, Render rebuilt ~1 min). **Live smoke test 2026-09-15: all 200** (filtered GET, trial-balance, summary; plain GET 200; no-key 401 auth gate intact). Phone should now load Reports + create entries. The DNS known-IP fallback from the P1 fix is untouched.
